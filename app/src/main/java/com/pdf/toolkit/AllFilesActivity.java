...
public class AllFilesActivity extends AppCompatActivity {
    ...
    private void loadPdfFiles() {
        ...
        new Thread(() -> {
            ...
            // Logic to scan for files (same as before)
            ...
            try (Cursor cursor = getContentResolver().query(queryUri, projection, selection, selectionArgs, null)) {
                if (cursor != null) {
                    ...
                    while (cursor.moveToNext()) {
                        newFileList.add(new FileItem(cursor.getString(nameColumn), cursor.getString(pathColumn), cursor.getLong(sizeColumn), cursor.getLong(dateColumn) * 1000));
                    }
                }
            }
            Collections.sort(newFileList, (f1, f2) -> Long.compare(f2.lastModified, f1.lastModified));
            
            runOnUiThread(() -> {
                progressBar.setVisibility(View.GONE);
                fileList.clear();
                fileList.addAll(newFileList);
                adapter.notifyDataSetChanged(); // This correctly refreshes the list
            });
        }).start();
    }

    // (The permission checking methods are the same)
